(ns laundrycoord.governor
  "LaundryCoordGovernor — the independent safety/scope layer gating
  every plant scheduling/logistics proposal an advisor may make for
  an industrial laundry crew. The governor never dispatches hardware
  itself, never operates laundry-machine equipment itself, and never
  ...
  press safe to operate, or a laundry batch safe for shipment), and
  never overrides a plant safety officer's judgment — those are
  permanently out of this actor's scope and remain a plant safety
  officer's exclusive judgment (README's 'Robotics premise': this
  actor coordinates PLANT SCHEDULING/LOGISTICS ONLY — it never
  operates laundry-machine equipment itself). Modeled closely on
  cloud-itonami-isco-8152's millcoord.governor.

  HARD invariants (:hard? true, ALWAYS :hold, never overridable):
    1. launderer provenance   — the crew member must be independently
                                verified/registered before any action.
    2. plant provenance       — the laundry plant/line must be
                                independently verified/registered
                                before any action.
    3. no-actuation           — proposal :effect must be :propose (the
                                governor never dispatches hardware and
                                never operates laundry-machine
    ...
    5. scope-excluded action  — any proposal to directly finalize a
                                ...
                                specific washing, drying or pressing
                                run), or a plant-safety-clearance
                                decision (e.g. declaring a washer,
                                dryer or press safe to operate, or a
                                laundry batch safe for shipment), or to
  ...
  regardless of confidence):
    6. :op :flag-safety-concern (a heat/steam-burn hazard,
                                entanglement-hazard from rotating
                                drums/pressing mechanisms, or
                                detergent/chemical-exposure concern
  ...
  operates laundry-machine equipment itself, and it never makes a
...
(def ^:private scope-excluded-ops
  #{:finalize-laundry-machine-operation :finalize-washing-operation
    :finalize-drying-operation :finalize-pressing-operation
    :authorize-washer-run :authorize-dryer-run :authorize-press-run
    :proceed-with-washing-run :proceed-with-drying-run :proceed-with-pressing-run
    :finalize-plant-safety-clearance
    :declare-machine-safe-for-operation
    :declare-laundry-batch-safe-for-shipment
...
;; ("detergent", "steam", "drum", "heat", "chemical", "washer",
;; "dryer", "press", "plant", "laundry", "safety", "officer") — so
...
;; "washing/drying/pressing task" / "plant safety officer review").
;; See
;; `governor-test/default-mock-advisor-proposals-never-self-trip-on-scope-exclusion`.
(def ^:private scope-excluded-phrases
  ["proceed with the washing run" "proceed with the drying run" "proceed with the pressing run"
   "authorize the washer run" "authorize the dryer run" "authorize the press run"
   "finalize the washing decision" "finalize the washing operation"
   "finalize the drying operation" "finalize the pressing operation"
   "finalize the laundry-machine operation" "finalize the laundry machine operation"
   "declare the machine safe for operation" "declare the washer safe for operation"
   "declare the dryer safe for operation" "declare the press safe for operation"
   "declare the laundry batch safe for shipment" "declare the batch safe for shipment"
   "clear the batch for shipment" "clear the laundry batch for shipment"
...
(defn- hard-violations [proposal launderer-record plant-record]
  (let [{:keys [op rationale]} proposal]
    (cond-> []
      (nil? launderer-record)
      (conj {:rule :no-launderer
             :detail "未登録 launderer への提案は不可（launderer record は独立して検証・登録済みでなければならない）"})
      ...
      (conj {:rule :no-actuation
             :detail "effect は :propose のみ許可（governor は洗濯・乾燥・プレス作業を直接実行しない）"})
      ...
      (conj {:rule :scope-excluded-action
             :detail "機械運転実行判断・プラント安全(plant-safety)クリアランス判断の確定、および plant safety officer の判断の上書きは、この actor の権限外 — 常に永続ブロック"}))))

(defn check
  "Assess a proposal against `request`/`context`/`proposal` and a
  `store` implementing `laundrycoord.store/Store`. Pure — never
  mutates the store, never dispatches a washing/drying/pressing
  operation."
  [request _context proposal store]
  (let [launderer-record (store/launderer store (:launderer-id request))
        plant-record (some->> (:plant-id proposal) (store/plant store))
        hard (hard-violations proposal launderer-record plant-record)
