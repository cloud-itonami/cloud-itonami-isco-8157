(ns laundrycoord.actor
  "LaundryCoordActor — the ISCO-08 8157 laundry machine operators
  plant scheduling/logistics coordination actor as a
  ...
  cloud-itonami-isco-8152's millcoord.actor.
  ...
  The unconditional invariant: the Laundry Plant Scheduling
  Coordination Advisor can never directly commit an operating record
  the LaundryCoordGovernor refuses, and can never make this actor
  ...
  can never override a plant safety officer's judgment — every
...
(defn build-graph
  "Build a compiled LaundryCoordActor graph. `store` implements
  `laundrycoord.store/Store`. `advisor` implements
  `laundrycoord.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:launderer-id (:launderer-id request)
                                    :op (:op proposal)
